...
class SerialClient(QObject):
    ...
    def __init__(self, baud=115200, parent=None, bridge=None):
        ...
        self._shared = bridge is not None
        self.port_name = None
        ...
        if self._shared:
            ...
            self.bridge.line.connect(self._on_shared_line)
        else:
    ...
    def open_port(self, name: str) -> bool:
        ...
        if self.uart.isOpen(): self.uart.close()
    ...
    def write_line(self, s: str):
        if self._shared:
            try:
                ...
                pass
            return
        if not self.uart.isOpen(): return
    ...
    def _on_shared_line(self, raw_str: str):
        # Filtre strict '#pi'
...
class AntCell(QFrame):
    def __init__(self, index: int, on_click, parent=None):
        ...
        self.label_index.setStyleSheet("font-size: 20px; font-weight: 600;")
        self.label_index.setAttribute(Qt.WA_TransparentForMouseEvents)

    def resizeEvent(self, e):
        self.label_index.setGeometry(self.rect())
        super().resizeEvent(e)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, bridge=None):
        ...
        self.serial.antChanged.connect(self._highlight)
        ...
        if bridge is None:
            ...
            self.lbl_status.setText("Partagé (géré par Afficheur)")
    ...
    def _build_ui(self):
        ...
        gb_log = QGroupBox("Console UART"); right.addWidget(gb_log, 1)
        v = QVBoxLayout(gb_log)
        self.log = QPlainTextEdit(); self.log.setReadOnly(True); self.log.setMaximumBlockCount(4000)
        v.addWidget(self.log)
        self.btn_clear = QPushButton("Clear"); self.btn_clear.clicked.connect(self.log.clear)
    ...
    def _refresh_ports(self):
        ...
        for p in available_ports():
            label = f"{p.portName()} — {p.description() or ''}".strip(" —")
    ...
    def _stop_uart_logs(self): self.serial.set_logging(False); self.append_log("# UART_RX LOGS: PAUSED (ANT/DEBUG masqués)")
    def _start_uart_logs(self): self.serial.set_logging(True);  self.append_log("# UART_RX LOGS: RUNNING")
    ...
    def on_line(self, line: str): self.append_log(line)
    def on_ant(self, n: int): self._highlight(n)

    def on_list_echo(self, values: list):
        present = set(values)
        for i, cb in enumerate(self.cbs_list, start=1):
            cb.blockSignals(True); cb.setChecked(i in present); cb.blockSignals(False)
        self._update_master_checkbox()
...
if __name__ == "__main__":
    ...
    win = MainWindow(bridge=None)  # bridge injecté par l'autre UI en pratique
    win.show()
    sys.exit(app.exec_())