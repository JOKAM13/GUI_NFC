...
class Afficheur(QtWidgets.QMainWindow):
    ...
    def _show_history_dialog(self, mouse_id, events):
        ...
        table.setHorizontalHeaderLabels(["Horodatage", "Zone", "Événement"])
...
class TrajectoryWidget(QtWidgets.QWidget):
    def __init__(self, parent=None):
        super().__init__(parent); self._points = []; self._seq = []; self._zones = []

    def set_events(self, events):
        seq_pts = []
        for ev in events:
            if ev.event in ("enter","stay"):
                z = ev.zone_idx; r, c = divmod(z, 5)
    ...
    def paintEvent(self, e):
        ...
        w, h = self.width(), self.height(); margin = 20
        grid_w, grid_h = w-2*margin, h-2*margin
        cols, rows = 5, 3; cw, rh = grid_w/cols, grid_h/rows
        p.fillRect(self.rect(), QtGui.QColor("#FFFFFF"))
        pen_grid = QtGui.QPen(QtGui.QColor("#3C3C3C")); pen_grid.setWidth(1); p.setPen(pen_grid)
        for r in range(rows):
            for c in range(cols):
                x = margin + c*cw; y = margin + r*rh
                p.drawRect(QtCore.QRectF(x, y, cw, rh))
        def center_of(r,c): return QtCore.QPointF(margin + c*cw + cw/2, margin + r*rh + rh/2)
        if len(self._points) >= 2:
            pen_path = QtGui.QPen(QtGui.QColor("#2D7FF9")); pen_path.setWidth(3); p.setPen(pen_path)
            for i in range(len(self._points)-1):
                p.drawLine(center_of(*self._points[i]), center_of(*self._points[i+1]))
        p.setPen(QtCore.Qt.NoPen); brush_node = QtGui.QBrush(QtGui.QColor("#78D46A"))
        for i, (r,c) in enumerate(self._points):
            center = center_of(r,c); radius = min(cw, rh)*0.12
            p.setBrush(brush_node); p.drawEllipse(center, radius, radius)
            p.setPen(QtGui.QPen(QtGui.QColor("#000"))); p.setFont(QtGui.QFont("", 10, QtGui.QFont.DemiBold))
